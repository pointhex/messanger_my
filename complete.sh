...
_my_command() { #объявляем функцию, которую будем привязывать к анализу
	...
	subcommands_1="get send clear init change"
	subcommands_2=$(cat /usr/vk/users) #массив подкоманд первого уровня - см. синтаксическое дерево в начале поста.
	...
	if [ ${COMP_WORDS[1]} == 'send' ] || [ ${COMP_WORDS[1]} == 'clear' ] || [ ${COMP_WORDS[1]} == 'change' ] ; then
